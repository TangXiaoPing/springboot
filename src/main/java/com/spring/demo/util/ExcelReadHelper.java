...
public class ExcelReadHelper {
    ...
    public static List<Object> excelRead(InputStream stream, String[] properties, Class obj) throws Exception {
        Workbook book = null;
        try {
            book = new XSSFWorkbook(stream);     //解析2003
        } catch (Exception e) {
            book = new HSSFWorkbook(stream);      //解析2007
        }
    ...
    public static List<Object> excelReadForPosition(InputStream stream, String[] properties, Class obj, Integer[] position) throws Exception {
        Workbook book = null;
        try {
            book = new HSSFWorkbook(stream);     //解析2003
        } catch (Exception e) {
            book = new XSSFWorkbook(stream);      //解析2007
        }

        return getExcelContentForPosition(book, properties, obj, position);
    }
    ...
    private static List<Object> getExcelContentForPosition(Workbook book, String[] properties,
                                                           Class obj, Integer[] position) throws Exception {
        List<Object> resultList = new ArrayList<Object>();        //初始化结果解
        Map<String, Method> methodMap = getObjectSetterMethod(obj);
        Map<String, Field> fieldMap = getObjectField(obj);
        for (int numSheet = 0; numSheet < book.getNumberOfSheets(); numSheet++) {
            ...
            for (int numRow = 1; numRow <= sheet.getLastRowNum(); numRow++) {   //一个row就相当于一个Object
                ...
                resultList.add(getObjectForPosition(row, properties, methodMap, fieldMap, obj, position));
            }
        }
        return resultList;
    }
    ...
    private static Object getObject(Row row, String[] properties,
        ...
        for (int numCell = 0; numCell < row.getLastCellNum(); numCell++) {
            ...
            if (cell == null || numCell >= properties.length) {
                continue;
            }
            String cellValue = getValue(cell).trim();
    ...
    // 获取指定列的数据，返回object对象
    private static Object getObjectForPosition(Row row, String[] properties, Map<String, Method> methodMap,
                                               Map<String, Field> fieldMap, Class obj, Integer[] position) throws Exception {
        Object object = obj.newInstance();
        for (int i = 0; i < position.length; i++) {
            Integer numCell = position[i];
            Cell cell = row.getCell(numCell);
            if (cell == null || i >= properties.length) {
                return null;
            }
        ...
        if ("java.lang.String".equals(type) || "String".equals(type)) {
            ...
            if (value.length() > 0) {//当长度为19(yyyy-MM-dd HH24:mm:ss)或者为14(yyyyMMddHH24mmss)时Date格式转换为yyyyMMddHH24mmss
                if (value.length() == 19 || value.length() == 14) {
                    oo[0] = DateUtil.parseToDate(value, DateUtil.YYYY_MM_DD_MM_HH_SS);
                } else {     //其余全部转换为yyyyMMdd格式
                    oo[0] = DateUtil.parseToDate(value, DateUtil.YYYY_MM_DD_MM_HH_SS);
                    //oo[0] = HSSFDateUtil.getJavaDate(Double.valueOf(value));
                }
            }
    ...
    private static String getValue(Cell cell) {
        if (cell.getCellType() == cell.CELL_TYPE_BOOLEAN) {
            ...
            if (org.apache.poi.ss.usermodel.DateUtil.isCellDateFormatted(cell)) {
                return String.valueOf(DateUtil.formatDate(cell.getDateCellValue(), DateUtil.YYYY_MM_DD_MM_HH_SS));
            }
    ...
    public static Workbook getWorkBook(InputStream in, String fileName) throws Exception {
        ...
        if (EXCEL2003.equals(fileType)) {
            book = new HSSFWorkbook(in);     //解析2003

        } else if (EXCEL2007.equals(fileType)) {
            book = new XSSFWorkbook(in);      //解析2007
        } else {
    ...
    /**
     * 描述：获取IO流中的数据，组装成List<List<Object>>对象，导入2003版Excel
     *
     * @param in fileName
     * @return
     * @throws Exception
     * @author pijinlin
     */
    public static List<Map<String, Object>> getBantchByExcel(InputStream in, String fileName, String[] clumnName) throws Exception {
            ...
            if (sheet == null) {
                ...
                for (int j = sheet.getFirstRowNum() + 1; j <= sheet.getLastRowNum(); j++) {
                    ...
                    if (row == null) {
                        ...
                        for (int y = 0; y < clumnName.length; y++) {
                            cell = row.getCell(y);
                            //判断该列是否有状态码
                            if (clumnName[y].indexOf(":") == -1) {
                                String key = clumnName[y];
                                if (cell == null) {
                                    map.put(key, "");
                                    continue;
                                }
                                ...
                                String[] split = clumnName[y].split(":");
                                String key = split[0];
                                if (cell == null) {
                                    map.put(key, "");
                                    continue;
                                }
                                String valueStr = (String) getCellvalue(cell);
                                for (int m = 1; m < split.length; m++) {
                                    String[] split1 = split[m].split("-");
                                    if (valueStr.equals(split1[1])) {
    ...
    private static Object getCellvalue(Cell cell) {
        ...
        switch (cell.getCellType()) {
            ...
            case Cell.CELL_TYPE_BLANK:
                ...
                break;
            default:
                value = StringUtil.replaceBlank(String.valueOf(cell.getStringCellValue()));
                break;
        }
        return value;
    }
    ...
    public void test() throws Exception {
        ClassPathResource classPathResource = new ClassPathResource("/static/test.xls");
        InputStream backStream = classPathResource.getInputStream();
        List<Map<String, Object>> data = getBantchByExcel(backStream, classPathResource.getFilename(), new String[]{"name", "sex", "age"});
    ...
    public void test2() throws Exception {
        ClassPathResource classPathResource = new ClassPathResource("/static/test.xls");
        InputStream backStream = classPathResource.getInputStream();
        List<Map<String, Object>> data = getBantchByExcel(backStream, classPathResource.getFilename(), new String[]{"name", "sex:1-男:0-女", "age"});
        System.out.println(data);
    }
}